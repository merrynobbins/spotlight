...
class ModelFactory:
    ...
    def to_track_model(self, track, session):
        ...
        track_artist_names = [artist.name() for artist in track.artists() if track is not None]
    ...
    def to_inbox_model(self, items, session):
        ...
        playlists = [Playlist(playlist.create(session, track_link)) 
                     for track_link in track_links if track_link.type() is LinkType.Playlist and track_link is not None]
